...
def compute_batch_EM(
    ground_truth_list: List[str],
    generation_list: List[str],
    language="python"
):
    gt_res = ground_truth_list
    pred_res = generation_list
    em_val = 0
    for gt_str, pred_str in zip(gt_res, pred_res):
        em_val += compute_EM(gt_str, pred_str, language=language)
    return em_val / len(pred_res)


def compute_batch_ES(
    ground_truth_list: List[str],
    generation_list: List[str],
    language="python"
    ):
...
def compute_identifier_match(prediction, target, language="python") -> tuple[int, float]:
...
def compute_bath_identifier_match(
    ground_truth_list: List[str],
    generation_list: List[str],
    language="python"
):
