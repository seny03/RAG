...
class projectSearcher(object):
    """
    A class for analyzing and extracting information about a Python project, including its structure, imports, and relationships between modules.
    Methods:
        __init__():
            Initializes the projectSearcher instance, loading standard modules from a JSON file.
        set_proj(proj_dir, proj_info):
            Sets the project directory and project information.
        name_in_file(name, avail_list, src_name=None, cls=None):
            Finds the available name in the current file.
        is_local_import(fpath, imported_info):
            Determines if a module or name is imported from the current project.
        get_distance_fpaths(src_path, target_path):
            Calculates the distance between two file paths based on their module hierarchy.
        _check_local_import(fpath, imported_info):
            Checks if a module or name is imported locally within the project.
        _get_indent(def_stat):
            Retrieves the indentation of a given definition statement.
        _get_module_prompt(file_info, name_set, only_def=True, enable_docstring=True):
            Generates a prompt for a module based on its file information and required names.
        _get_class_prompt(file_info, cls_name, cls_dict, name_set, only_def=True, enable_docstring=True):
            Generates a prompt for a class, including its members and docstring.
        _get_function_prompt(node_info, only_def=True, enable_docstring=True):
            Generates a prompt for a function, including its definition and optional docstring.
        _get_variable_prompt(file_info, name_list, only_def=True):
            Generates a prompt for variables, optionally including their definitions.
        get_path_comment(fpath):
            Generates a comment string for a given file path.
        get_prompt4names(fpath, name_set, only_def=True, enable_docstring=True):
            Generates a prompt for specific names in a file, merging related statements, functions, classes, or modules.
        pseudo_topo_sort(fpath_set, file_edges, fpath_order):
            Performs a pseudo topological sort on a set of file paths based on their dependencies.
        depthFirstSearch(fpath, name, max_hop=None):
            Performs a depth-first search starting from a specific file and name to explore related nodes.
        dfs(fpath, name, depth, node_dict, file_edges, max_hop):
            Helper method for depth-first search to traverse the project information graph.
        get_prompt(node_list, max_hop=None, only_def=True, enable_docstring=True):
            Generates a comprehensive prompt for a list of nodes, including their dependencies and relationships.
    """
    ...
    def get_prompt_list(self, node_list, max_hop=None, only_def=True, enable_docstring=True) -> list[str]:
        '''
        ...
        return [item.replace("'''", '"""') for item in prompt_list]
